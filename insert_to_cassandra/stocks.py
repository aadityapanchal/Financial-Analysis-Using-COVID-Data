...
def main():
    ...
    #df.show(10000)
    #df.write.format("org.apache.spark.sql.cassandra").options(table='stocks', keyspace='dataflix').mode('overwrite').save()
    df.write.format("org.apache.spark.sql.cassandra").mode("overwrite").option("confirm.truncate","true").options(keyspace='dataflix',table='stocks').save()
