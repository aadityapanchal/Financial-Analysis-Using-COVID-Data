...
import load_covid, load_stocks

def main():

    # USDRUB_df, USDGBP_df, USDCHF_df, USDCAD_df, USDEUR_df, USDJPY_df, USDAUD_df, USDBRL_df, USDINR_df, USDMXN_df = load_stocks.get_forex_data(spark)
    ...
    stocks = ['ndx_composite', 'rut', 'dow_jones', 'ndx', 'spx', 'nyse']

    for name in stocks:
        df_stocks = load_stocks.get_forex_data(spark, name)
        join_covid19_stocks = covid19_df.join(df_stocks, ['date'])
        # df_stocks.show()
        join_covid19_stocks.show(1)

        train, validate = join_covid19_stocks.randomSplit([0.75, 0.25])
        train = train.cache()
        ...
        model.write().overwrite().save('models/stocks/' + name)
